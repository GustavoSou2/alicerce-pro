...
@Component({
  ...
  template: `
    <section class="select-component">
      ...
      <div class="select-container" #selectTrigger (click)="this.fetchData()" #selectContainer>
        <input
          type="text"
          class="field"
...
export class SelectComponent implements ControlValueAccessor {
  ...
  set setStateDropDown(state: boolean) {
    const triggerRect: any = this.selectTriggerRef?.nativeElement?.getBoundingClientRect();
  ...
  filterItems() {
    this.filteredData = this.data.filter((item) => {
      ...
      if (typeof display == 'string')
        return (<string>display).toLocaleLowerCase().includes(this.searchQuery.toLowerCase());
  ...
  writeValue(value: any): void {
    ...
    if (this.data.length) {
      ...
      const selectedItem = this.data.find((item) => item[this.source.keys.value] == value);
